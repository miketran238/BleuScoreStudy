...
Because the BLEU metric measures the the phrase-to-phrase translation while source code has well syntactic and semantics, we hypothesize that {\em BLEU metric is not effective in evaluating the translated results of migrated source code}. The hypothesis can further be divided into two parts: BLEU does not reflect well the semantic accuracy of migrated source code, and BLEU does not work well for the task of comparing different SMT models.  We
...
because it is a popular SE task that applies SMT. In the domain of migration task that used SMT, we pick the three most popular models: lpSMT, mppSMT, and GNMT. To prove the first part of our hypothesis, we show that BLEU has weak correlation with human judgments on translated source code in term of functionality. For the second part, we show that an improvement in BLEU is not sufficient nor necessary to reflect an improvement in code migration quality.
...
In our experiment, we used a dataset of {\bf 34,209} pairs of methods
...
semantic accuracy score was given by a developer after
...
weak correlation to the semantic accuracy of the migrated code. Additionally, we compared the translated results of the same set of 375 methods for two different SMT models. In nearly half of those methods, an improvement of BLEU score does not indicate an improvement in semantic score, and vice-versa.
%Tien
...
the resulting migrated code. {\model} can also be used to compare different SMT models as it can predict successfully (95\% of the cases) the change in translation quality (measured by semantic score). The contributions of this paper include:
