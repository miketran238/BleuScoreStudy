...
Because the BLEU metric and programming languages have different
nature in which BLEU measures the phrase-to-phrase translation and
source code has well syntactic and semantics, we hypothesize that {\em
  BLEU metric does not reflect well the semantic accuracy on the
  migrated source code}. We conducted experiments to provide
counterexamples to empirically validate that hypothesis. We choose the
language migration task from Java to C\texttt{\#} because it is the
most popular SE task that applies SMT. We show that under some
circumstances an improvement in BLEU is not sufficient to reflect an
improvement in code migration quality, and in other circumstances that
it is not necessary to improve BLEU in order to achieve an improvement
in migration quality. 
...
picked mppSMT tool~\cite{ase15} that high semantic accuracy but
with a wide range of BLEU~scores.

In our experiment, we used a dataset of {\bf 30,000} pairs of methods
in {\bf 9} projects that were manually migrated from Java to
...
those methods. We then manually investigated {\bf 367}
...
semantic saccuracy score was given by an author of the paper after
examining the original code in Java, the manually migrated code in
C\texttt{\#} and the auto-migrated code in C\texttt{\#} by a tool. We
then computed the correlation between the BLEU scores and semantic
scores of all the pairs. Our result shows that the BLEU metric has a
weak correlation to the semantic accuracy of the migrated code.
However, the result indicates that the BLEU metric is correlated with
syntactic correctness of the migrated~code. Thus, BLEU could be used
to evaluate the improvement of the models on the syntactic
accuracy of the resulting migrated code.
...
graph (PDG) as data and control dependencies aamong program entities
...
highly correlated to the human judgements on the semantic accuracy of
the resulting migrated code. The contributions of this paper include:
...
migration tools. However, BLEU could be used to measure
syntactic correctness of the migrated code.
