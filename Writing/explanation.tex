...
results. In this work, we selected to investigate the subset with the 
\textbf{median} of the correlation, 0.955, with the size of 308.
...
different ways such as \code{part[k]} or using Iterator.
%
...
reference code. In function \code{TestParserGrammar} of
...
\code{ContentAppend} in the translated version is similar to the
reference one with the condition branches \code{if} and throwing
exception \code{IllegalPdfSyntaxException}. {\model} reflects this
...
\textbf{Subset 2}. In general, {\model} tries to measure the
