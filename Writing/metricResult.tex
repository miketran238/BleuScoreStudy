...
To evaluate the abilities of STS, TRS and GRS in reflecting the semantic accuracy, 
we conducted three experiments for these three metrics in the same manner as the 
previous study described in section~\ref{sec:bleuresult}. 

The correlation coefficient results between each of STS, TRS and GRS with semantic 
score are described in table \ref{table:correlation}. These results follow a
similar trend, which is for each model, the correlation coefficients
increase constantly according the abstraction levels of source code in three metrics.
For mppSMT as an example, the correlation coefficient between STS and semantic score 
is 0.549, whereas the figure for TRS is much greater at 0.820. The highest value is 
the correlation coefficient is between GRS, 0.910. A reason for this 
phenomenon is that for a pair of source code, if they are textually identical, 
they are uniform in their tree-based representations, and the duplication
in ASTs leads to the equivalence between PDGs. Meanwhile, the equivalence in higher 
abstraction does not necessarily lead to the equivalence in lower abstraction. 
For example, figure \ref{fig:mppSMT_example} shows two methods that have as
two equivalent PDGs, but they are textually different.

Based on these empirical results, we conclude that the metric that measures the results 
in the higher abstraction level, the better metric for reflecting the semantics accuracy.
However, the higher representation like PDG is not always constructed because of missing
required information in the translated results. Therefore, in section \ref{sec:proposal}, 
we propose a metric to evaluate the quality of translated code, that combines the measuring 
the similarity of translated code and expected results in multiple representations.
