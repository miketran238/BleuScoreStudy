...
in Chris {\em et al.}~\cite{Callison}. The idea is that BLEU {\em does not impose
explicit constraints on the orders of all matching phrases} between the
...
$t_1$: "\textbf{A B} E \textbf{C D}", $t_2$: "\textbf{C D} E \textbf{A B}" 
are the same, because the number of matching phrases such as "A B" and "C D" 
...
similar BLEU scores with the translated one. However, for code migrated results,
the difference in the order of phrases between two results might lead to the 
considerable difference between them in term of semantic. For instance, if 
"\textbf{A B}", "\textbf{E}", and "\textbf{C D}" represent for the blocks code of 
opening a file, reading the opened file, and closing the opened file, $t_1$ and 
$t_2$ are significantly different in term of functionality. Hence, {\em by 
following that  principle, the results from p-mppSMT achieve BLEU score equivalent 
