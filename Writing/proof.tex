...
Specifically, that is, the equivalence of BLEU score of the results translated 
by two models is not consistent with the equivalence in the translation quality 
of these models.
%

To provide the counterexample, in this work, we construct a permuted model, p-mppSMT, 
from mppSMT, that is applied the principle introduced by Chris et al. \cite{Callison}. 
They argue that BLEU places confused constraints on the order that matching 
n-grams occur in the translated result and the reference one. This leads 
to that for a given reference code and a translated result, there are many
possible variants that would have similar BLEU score with the translated one.
By following that principle, the results translated by our model archive the 
identical BLEU score with the results of mppSMT, but possibly get lower semantic score.
