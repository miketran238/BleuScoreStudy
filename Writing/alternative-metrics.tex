...
distance~\cite{??}, tree edit distance~\cite{??}, and graph edit
distance~\cite{??}, respectively.
...
$$TRS(R, T) = 1 -\frac{TED(AST_R, AST_T)}{max\left(CountNodes(AST_R),
  CountNodes(AST_T)\right)}$$ where $TED(AST_R, AST_T)$ is the edit
...
identical \cite{oopsla10}; and $CountNodes(t)$ returns the number of
nodes in the AST $t$.
%
