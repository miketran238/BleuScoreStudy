...
$$STS = 1 - \frac{SED(S_R, S_T)}{max\left(length(S_R), length(S_T)\right)}$$
...
the similarity between two ASTs representing for the translated code 
and the reference one:
$$TRS = 1 -\frac{TED(AST_R, AST_T)}{max\left(CountNodes(AST_R), CountNodes(AST_T)\right)}$$  
...
In this work, we define \textbf{graph similarity (GRS)} between the translated 
result and the reference code represented as program dependence graphs 
by using graph edit distance \cite{??}. That is:
$$1-\frac{GED(PDG_R, PDG_T)}{max\left(Size(PDG_R), Size(PDG_T)\right)}$$
